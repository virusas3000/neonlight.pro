...
<div class="nl-page nl-shop-page">
	<div class="nl-shop-header">
		<nav class="nl-breadcrumb"><a href="<?php echo esc_url(home_url('/')); ?>"><?php echo nl_t('shop_breadcrumb_home'); ?></a> / <span><?php echo nl_t('neon_products_title'); ?></span></nav>
		<h1 class="nl-shop-title"><?php echo nl_t('neon_products_title'); ?></h1>
		<div class="nl-shop-toolbar">
			...
			<div class="nl-sort-dropdown">
				<select><option><?php echo nl_t('shop_sort_default'); ?></option></select>
			</div>
		</div>
	</div>
	...
	<?php if ($products->have_posts()) : ?>
		<div class="nl-product-grid">
			<?php while ($products->have_posts()) : $products->the_post(); ?>
				...
				if (!$product || is_wp_error($product)) continue;
				?>
				<div class="nl-product-card">
					<?php if ($product->is_on_sale()) : ?>
						<div class="nl-sale-badge"><?php echo nl_t('shop_sale'); ?></div>
					<?php endif; ?>
					<a href="<?php echo esc_url(get_permalink() . '?lang=' . $lang); ?>" class="nl-product-card__link">
						...
						<div class="nl-product-card__price">
							...
							if ($product->is_on_sale() && $sale > 0) {
								echo '<span class="nl-product-card__price-regular">$' . number_format($reg,2) . '</span>';
								echo '<span class="nl-product-card__price-sale">$' . number_format($sale,2) . '</span>';
							} elseif ($reg > 0) {
								echo '<span class="nl-product-card__price-current">$' . number_format($reg,2) . '</span>';
							} else {
								echo '<span class="nl-product-card__price-current">' . nl_t('neon_products_coming') . '</span>';
							}
							?>
						</div>
					</a>
	...
	<?php else : ?>
		<div style="text-align:center; padding:60px 20px;">
			<p><?php echo nl_t('neon_products_coming'); ?></p>
		</div>
	<?php endif; ?>
</div>

<style>
/* Neon Products page — mirrors Rental exactly */
...
.nl-btn-primary { width:100%; padding:10px; border:none; border-radius:30px; background:#00d4b0; color:#fff; font-size:14px; cursor:pointer; }
.nl-btn-primary:hover { background:#00bfa0; }
